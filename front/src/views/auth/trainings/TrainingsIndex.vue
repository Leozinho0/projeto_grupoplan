    ...
    <div v-else class="root">
      <v-row class="item">
        <v-col class="col-12">
          <v-card flat>
            <v-card-text>
              <span class="display-2 color-base--text">Treinamentos</span>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
                ...
                <v-col class="col-12">
                  <v-data-table
                    ...
                    <template v-slot:item.uuid="{ item }">
                      <span>{{ item.uuid | truncate(10) }}</span>
                    </template>
                    <template v-slot:item.name="{ item }">
                      <span class="blue--text">{{ item.name | truncate(20) }}</span>
                    </template>
                      ...
                      <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                          <v-btn 
                            ...
                            v-bind="attrs"
                            icon
                            :to="{ name: 'TrainingsEdit', params: { training: item.uuid } }">
                            <v-icon
                              color="green">
                              mdi-pencil
                            </v-icon>
    ...
    data() {
      return {
        ...
        breadcrumbs: [ 
          ...
          { id: 2, to: { name: 'TrainingsIndex' }, disabled: true, text: 'Treinamentos' }, 
        ],
    ...
    methods: {
      ...
      async paginate() {
        ...
        try {
          ...
          const response = await Training.index(params);
